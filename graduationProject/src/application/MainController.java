...
public class MainController implements Initializable{
	...
	public static List<File> newEncodingFiles = new ArrayList();
	public static List<File> newSelectedFiles = new ArrayList();
	...
	List<Object> timePiece = new ArrayList<Object>();
	List<String> fileNameTimeStamp = new ArrayList<String>();
	List<String> conversionTimeStamp = new ArrayList<String>();
	List<Object> selectedTime = new ArrayList<Object>();
	@FXML 
	...
	@FXML
	private ListView <Object>timeList;
	@FXML
			...
			public void handle(MouseEvent event) {
				...
				if (fileFlag == 1) {
					...
					for(int i=0; i<timeStamp.size(); i++) {
						double hour=(double) timeStamp.get(i) / 3600;
						double minute=(double) timeStamp.get(i) % 3600 / 60;
						double second=(double) timeStamp.get(i) % 60 ;
						fileNameTimeStamp.add(i, String.format("%3d:%02d:%04.1f",(int)hour, (int)minute,second));
					}
					...
					newEncodingFiles.clear();
					timeStamp.clear();
					fileNameTimeStamp.clear();
					
					for(File file : encodingFiles) {
						selectedFile = selectedFiles.get(num);
						timePiece = ReadJSONFile.getTimeStamp(file,word);

						if(!timePiece.isEmpty()) {
							newSelectedFiles.add(selectedFile);
							newEncodingFiles.add(file);
							for(int m=0; m < timePiece.size(); m++) {
								timeStamp.add(timePiece.get(m));
								double hour=(double) timePiece.get(m) / 3600;
								double minute=(double) timePiece.get(m) % 3600 / 60;
								double second=(double) timePiece.get(m) % 60 ;
								timePiece.set(m, String.format("%3d:%02d:%04.1f",(int)hour, (int)minute,second));
								fileNameTimeStamp.add(selectedFile.getName() + "   " + timePiece.get(m));
		...
		timeList.setOnMouseClicked(new EventHandler<MouseEvent>() {  //시간 선택시 동영상 이동
			...
			public void handle(MouseEvent event) {
				...
				if (fileFlag == 2) {
					mediaPlayer.stop();
					int num = 0;
					for(File file : newEncodingFiles) {
						selectedTime = ReadJSONFile.getTimeStamp(file, word);
						for(int m=0; m < selectedTime.size(); m++) {
							if(time == (double)selectedTime.get(m)) {
								selectedFile = newSelectedFiles.get(num);
								encodingFile = newEncodingFiles.get(num);
							}
						}
						num++;
					}
	...
	public void textload() throws Exception {  //텍스트 불러오기
		if(toggle.isSelected()==false) { // 버튼이 눌려졌을 때
	...
	public void showTimeStamp() {  //타임스탬프 출력하기
		...
		if(fileFlag == 1) {
			for(int i=0; i<timeStamp.size() ; i++) {  //타임스탬프 출력
				timeList.getItems().addAll(fileNameTimeStamp.get(i));
			}
		}
		else if(fileFlag == 2) {
			for(int i=0; i<fileNameTimeStamp.size() ; i++) {  //타임스탬프 출력
				timeList.getItems().addAll(fileNameTimeStamp.get(i));
			}
		}
	...
	public static String convert(File f) throws IOException { // 인코딩 
		String ffmpegPath = "D:\\Downloads\\ffmpeg-4.1.1-win64-static\\bin/ffmpeg";    //예) /work/ffmpeg
